...
// جلب بيانات الكلية مع دعم الكليات المستقلة
$college_sql = "SELECT c.*, 
...
$stmt = $conn->prepare($imgs_sql);
$stmt->bind_param("i", $college_id);
$stmt->execute();
$imgs_result = $stmt->get_result();
...
// إحصائيات الكلية
$stats_sql = "SELECT
    (SELECT COUNT(*) FROM specializations WHERE college_id = ? AND status = 'متاح') as total_specializations";
...
<head>
    ...
    <title><?php echo htmlspecialchars($college['name']); ?> - بوابة الجامعات اليمنية</title>
    <meta name="description" content="<?php echo htmlspecialchars(substr(trim($college['description'] ?? 'كلية في اليمن'), 0, 160)); ?>">
            ...
            <div class="header-content">
                ...
                <div class="university-hero">
                        ...
                        <div class="university-details">
                            ...
                            <div class="university-meta">
                                <?php if ($college['university_name']): ?>
                                    <span class="university-type <?php echo $college['university_type'] == 'حكومية' ? 'government' : 'private'; ?>">
                                ...
                                <?php else: ?>
                                    <span class="university-type government">
                                        حكومية
                                    </span>
                                    <span class="university-location">
                    ...
                    <div class="university-stats">
                        ...
                        <div class="stat-item">
                            <div class="stat-number"><?php echo $college['status'] === 'نشطة' ? 'نشطة' : 'غير نشطة'; ?></div>
                            ...
                            <?php foreach ($valid_images as $index => $img): ?>
                                <div class="slide" data-index="<?php echo $index; ?>">
                                    <img src="<?php echo htmlspecialchars($img['image_path']); ?>" 
                                         alt="<?php echo htmlspecialchars($img['alt_text'] ?: $college['name']); ?>" 
                                         loading="lazy"
                                         decoding="async" 
                                    ...
                                    <div class="slide-overlay">
                                        <div class="slide-info">
                                            ...
                                            <span class="slide-title"><?php echo htmlspecialchars($img['alt_text'] ?: $college['name']); ?></span>
                                        </div>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                ...
                <section class="info-section">
                    ...
                    <div class="info-card">
                        <div class="info-content">
                            <p class="description">
                                <?php 
                                if (!empty($college['description'])) {
                                    // تنظيف النص من المسافات الزائدة
                                    $cleaned_description = preg_replace('/\s+/', ' ', trim($college['description']));
                                    echo nl2br(htmlspecialchars($cleaned_description));
                                } else {
                                    echo 'لا يوجد وصف متاح للكلية حالياً.';
                                }
                                ?>
                            </p>
                                ...
                                <div class="contact-grid">
                                    ...
                                    <div class="contact-item">
                                        ...
                                        <div>
                                            ...
                                            <span>
                                                <?php 
                                                if ($college['university_location']) {
                                                    echo htmlspecialchars($college['university_location']);
                                                } elseif ($college['location']) {
                                                    echo htmlspecialchars($college['location']);
                                                } else {
                                                    echo 'غير محدد';
                                                }
                                                ?>
                                    ...
                                    <div class="contact-item">
                                        ...
                                        <div>
                                            ...
                                            <a href="<?php echo htmlspecialchars($college['website']); ?>" target="_blank" rel="noopener">زيارة الموقع</a>
                                        </div>
                                    </div>
                                    <?php endif; ?>
                                    ...
                                    <div class="contact-item">
                                        ...
                                        <div>
                                            ...
                                            <a href="<?php echo htmlspecialchars($college['coordination_link']); ?>" target="_blank" rel="noopener">زيارة موقع التنسيق</a>
                                        </div>
                                    </div>
                                    <?php endif; ?>
                                </div>
...
<?php if (!empty($college['map_url'])): ?>
<section class="map-section">
    <div class="section-header">
        <h2>الموقع الجغرافي</h2>
        <p>موقع الكلية على الخريطة</p>
    </div>
        ...
        <div class="map-placeholder">
            ...
            <p>
                ...
                if ($college['university_location']) {
                    ...
                    echo 'الموقع غير محدد';
                }
                ?>
            </p>
            <div class="map-actions">
                <a href="<?php echo htmlspecialchars($college['map_url']); ?>" target="_blank" class="btn btn-primary">
...
<?php endif; ?>
                ...
                <!-- التخصصات -->
                <?php
                // جلب التخصصات التابعة لهذه الكلية
                $spec_sql = "SELECT id, name, description, duration, degree_type, admission_requirement, admission_prerequisites, status 
                            FROM specializations 
                            WHERE college_id = ? 
                            ORDER BY name";
                $spec_stmt = $conn->prepare($spec_sql);
                $spec_stmt->bind_param('i', $college_id);
                $spec_stmt->execute();
                ...
                <section class="colleges-section">
                    <div class="section-header">
                        ...
                        <p>جميع التخصصات الدراسية المتاحة في هذه الكلية</p>
                    </div>

                    <?php if ($spec_result->num_rows > 0): ?>
                        <div class="colleges-grid">
                            <?php while($spec = $spec_result->fetch_assoc()): ?>
                                <div class="college-card">
                                    <div class="college-header">
                                        ...
                                        <div class="college-title-section">
                                            <h3><?php echo htmlspecialchars($spec['name']); ?></h3>
                                            <span class="college-status status-<?php echo $spec['status'] === 'متاح' ? 'active' : 'inactive'; ?>">
                                                <?php echo htmlspecialchars($spec['status']); ?>
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="college-content">
                                        ...
                                        <p class="truncate-description">
                                            ...
                                            // تقصير الوصف إلى 80 حرف فقط (نصف سطر تقريباً)
                                        ...
                                        <div class="specialization-meta">
                                            <span class="duration-badge">
                                                ...
                                                <?php echo (int)$spec['duration']; ?> سنوات
                                            </span>
                                            <span class="degree-badge">
                                                <i class="fas fa-graduation-cap"></i>
                                                <?php echo htmlspecialchars($spec['degree_type']); ?>
                                            </span>
                                        </div>

                                        <?php if (!empty($spec['admission_requirement'])): ?>
                                        <div class="admission-info">
                                            <h4>شروط القبول:</h4>
                                            <p class="truncate-text"><?php 
                                                $req = trim($spec['admission_requirement']);
                                                // تنظيف النص من المسافات الزائدة
                                                $req = preg_replace('/\s+/', ' ', $req);
                                                if (function_exists('mb_substr')) {
                                                    echo htmlspecialchars(mb_substr($req, 0, 60, 'UTF-8') . (mb_strlen($req, 'UTF-8') > 60 ? '...' : ''));
                                                } else {
                                                    echo htmlspecialchars(substr($req, 0, 60) . (strlen($req) > 60 ? '...' : ''));
                                                }
                                            ?></p>
                                        </div>
                                        <?php endif; ?>

                                        <?php if (!empty($spec['admission_prerequisites'])): ?>
                                        <div class="prerequisites-info">
                                            <h4>المتطلبات السابقة:</h4>
                                            <p class="truncate-text"><?php 
                                                $pre = trim($spec['admission_prerequisites']);
                                                // تنظيف النص من المسافات الزائدة
                                                $pre = preg_replace('/\s+/', ' ', $pre);
                                                if (function_exists('mb_substr')) {
                                                    echo htmlspecialchars(mb_substr($pre, 0, 60, 'UTF-8') . (mb_strlen($pre, 'UTF-8') > 60 ? '...' : ''));
                                                } else {
                                                    echo htmlspecialchars(substr($pre, 0, 60) . (strlen($pre) > 60 ? '...' : ''));
                                                }
                                            ?></p>
                                        </div>
                                        <?php endif; ?>
                                    ...
                                    <div class="college-footer">
                                        <a href="specialization-details.php?id=<?php echo $spec['id']; ?>" class="btn btn-sm btn-primary">
                    ...
                    <?php else: ?>
                        <div class="no-results">
                            ...
                            <p>سيتم إضافة التخصصات قريباً</p>
                        </div>
                    <?php endif; ?>
                </section>
            </div>
